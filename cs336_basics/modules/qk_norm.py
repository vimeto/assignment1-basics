...
class QKNorm(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x: (B, H, S, D). Normalize along the last dim with RMSNorm kernel, then apply per-head gamma.
        # Use F.rms_norm with weight=None to compute pure normalization, then scale by our learnable weight.
